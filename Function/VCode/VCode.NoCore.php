...
class ValidateCode {
    ...
    public function createFont() {
        ...
        for ($i = 0; $i < $this->codelen; $i++) {
            ...
            imagettftext($this->img, $this->fontsize, mt_rand(0, 10), $_x * $i + mt_rand(1, 5), $this->height / 1.2, $this->fontcolor, $this->font, $this->code[$i]);
        }
    }
    ...
    function GIFAddFrames($i, $d) {
        ...
        if ($this->COL > -1 && ord($this->BUF[$i][10]) & 0x80) {
            for ($j = 0; $j < (2 << (ord($this->BUF[$i][10]) & 0x07)); $j++) {
                if (ord($Locals_rgb[3 * $j + 0]) == ($this->COL >> 0) & 0xFF && 
